...
class Solution {
  public:
    int i= 0;
    Node* Bst(int arr[], int size,int low = 0,int high = INT_MAX) {
        if(i == size or arr[i] < low or arr[i] > high)return NULL;
        Node* root = newNode(arr[i++]);
        root->left = Bst(arr,size,low, root->data);
        root->right = Bst(arr,size,root->data,high);
        return root;
    }
};
